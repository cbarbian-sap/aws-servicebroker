...
def get_cidrs(size, qty, vpc_id, range):
    ...
    unused_cidrs = (netaddr.IPSet(map(lambda x : x['CidrBlock'],vpc.cidr_block_association_set)) ^ allocated_cidrs) & netaddr.IPSet([range])
    available_cidrs = []
    ...
    try:
        if event['RequestType'] in ['Create', 'Update']:
            response_data['CidrBlocks'] = get_cidrs(
                ...
                event['ResourceProperties']['CidrRange'] if 'CidrRange' in event['ResourceProperties'] else '0.0.0.0/0'
            )
